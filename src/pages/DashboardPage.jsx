...
import {
  Eye, EyeOff, Bell, Headphones, ChevronRight, Send, Plus, CreditCard,
  Phone, Wifi, Zap, Gamepad2, PiggyBank, TrendingUp, ArrowDownLeft,
  Shield, Users, FileText, MoreHorizontal, ArrowUpRight, ArrowDownLeft as ArrowIn
} from "lucide-react";

const QUICK_ACTIONS = [
  { label: "To Bank", icon: "🏦", path: "/transfer/bank", bg: "bg-violet-100 dark:bg-violet-900/40" },
  { label: "To BytePay", icon: "💜", path: "/transfer/bytepay", bg: "bg-purple-100 dark:bg-purple-900/40" },
  { label: "Savings", icon: "🐷", path: "/savings", bg: "bg-pink-100 dark:bg-pink-900/40" },
  { label: "Add Money", icon: "💳", path: "/add-money", bg: "bg-indigo-100 dark:bg-indigo-900/40" },
];

const SERVICES = [
  { label: "Airtime", path: "/airtime", emoji: "📞", bg: "bg-blue-50 dark:bg-blue-900/30" },
  { label: "Data", path: "/data", emoji: "📶", bg: "bg-green-50 dark:bg-green-900/30" },
  { label: "Betting", path: "/betting", emoji: "⚽", bg: "bg-emerald-50 dark:bg-emerald-900/30" },
  { label: "Electricity", path: "/electricity", emoji: "⚡", bg: "bg-yellow-50 dark:bg-yellow-900/30" },
  { label: "Refer & Earn", path: "/dashboard", emoji: "🎁", bg: "bg-rose-50 dark:bg-rose-900/30" },
  { label: "Insurance", path: "/dashboard", emoji: "🛡️", bg: "bg-cyan-50 dark:bg-cyan-900/30" },
  { label: "Loan", path: "/wealth", emoji: "💰", bg: "bg-amber-50 dark:bg-amber-900/30" },
  { label: "More", path: "/dashboard", emoji: "⊞", bg: "bg-gray-50 dark:bg-gray-800/30" },
];
        ...
        <header className="bg-white dark:bg-card px-4 py-3 flex items-center justify-between">
          <Link href="/profile" className="flex items-center gap-2.5" data-testid="link-profile">
            {userData?.profileImg ? (
              ...
              <div className="h-11 w-11 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-base font-bold">{initials}</div>
            )}
            <div>
              <p className="text-[11px] text-muted-foreground">{greeting},</p>
              <p className="text-base font-bold text-foreground leading-tight">Hi, {userData?.firstName?.toUpperCase() ?? "USER"}</p>
            </div>
          </Link>
          <div className="flex items-center gap-2">
            <button onClick={toggleTheme} className="h-9 w-9 rounded-full bg-secondary flex items-center justify-center" data-testid="btn-toggle-theme">
            ...
            <button className="relative h-9 w-9 rounded-full bg-secondary flex items-center justify-center" data-testid="btn-notifications">
              ...
              <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[9px] font-bold flex items-center justify-center px-1">99+</span>
            </button>
          </div>
        </header>
        ...
        <div className="px-4 pb-28 pt-3 space-y-3">
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <button onClick={() => setShowBalance(!showBalance)} data-testid="btn-toggle-balance">
                  {showBalance ? <EyeOff className="h-3.5 w-3.5 text-muted-foreground" /> : <Eye className="h-3.5 w-3.5 text-muted-foreground" />}
                </button>
              </div>
              <Link href="/history" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary" data-testid="link-history">
                Transact...n History <ChevronRight className="h-3 w-3" />
              </Link>
            </div>
            ...
            <div className="flex items-end justify-between mb-3">
              <div className="flex items-center gap-2">
                <AnimatePresence mode="wait">
                  <motion.p key={showBalance ? "show" : "hide"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="text-3xl font-bold text-foreground" data-testid="text-balance">
          ...
          <div className="bg-white dark:bg-card rounded-2xl p-4 shadow-sm">
            <div className="grid grid-cols-4 gap-2">
              {QUICK_ACTIONS.map(({ label, icon, path, bg }, i) => (
                <motion.button key={path} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  onClick={() => setLocation(path)}
                  className="flex flex-col items-center gap-1.5"
                  data-testid={`action-${label.replace(/\s+/g, "-").toLowerCase()}`}>
                  <div className={`h-14 w-14 rounded-2xl ${bg} flex items-center justify-center text-2xl`}>
                    {icon}
                  </div>
            ...
            <div className="bg-white dark:bg-card rounded-2xl p-4 shadow-sm">
              <div className="flex items-center gap-3">
                <div className={`h-10 w-10 rounded-full flex items-center justify-center ${lastTx.type === "credit" ? "bg-green-100" : "bg-violet-100"}`}>
                  {lastTx.type === "credit"
                    ? <ArrowIn className="h-5 w-5 text-green-600" />
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {lastTx.date?.toDate ? lastTx.date.toDate().toLocaleDateString("en-NG", { month: "short", day: "numeric" }) : ""}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-card rounded-2xl p-4 shadow-sm">
            <div className="grid grid-cols-4 gap-3">
              {SERVICES.map(({ label, path, emoji, bg }, i) => (
                <motion.button key={label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  onClick={() => setLocation(path)}
                  className="flex flex-col items-center gap-1.5"
                  data-testid={`service-${label.toLowerCase().replace(/\s+/g, "-")}`}>
                  <div className={`h-12 w-12 rounded-2xl ${bg} flex items-center justify-center text-xl`}>
                    {emoji}
                  </div>
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm">
            <AnimatePresence mode="wait">
              <motion.div key={bannerIdx} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }}
                transition={{ duration: 0.3 }}
                className={`bg-gradient-to-r ${BANNERS[bannerIdx].bg} p-4 flex items-center justify-between min-h-[80px]`}>
                <div>
                  <span className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full font-bold uppercase">{BANNERS[bannerIdx].badge}</span>
                ...
                <div className="h-14 w-14 rounded-full bg-white/15 flex items-center justify-center text-3xl">💜</div>
              </motion.div>
            </AnimatePresence>
